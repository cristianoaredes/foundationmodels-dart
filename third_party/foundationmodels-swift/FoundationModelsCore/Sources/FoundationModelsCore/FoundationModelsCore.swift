...
private func processPCCEntitlementPresent() -> Bool {
    // SecTask* is exposed on macOS Security umbrella; iOS/iPhoneSimulator SDK
    // does not ship public SecTask headers (FND-0009 / TCK-0042). Fail closed.
#if os(macOS)
...
#else
        throw JsonRpcError.unsupported(
            "Apple's native vision tools are not available on this platform.",
            data: [
                "code": "SYSTEM_TOOL_UNAVAILABLE",
                "native": kind,
