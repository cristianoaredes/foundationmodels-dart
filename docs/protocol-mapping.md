...
| `sessions.stream` | `Session.stream(...)` → `Stream<StreamEvent>` (demux by `requestId`) | `respondStream(params:onEvent:)` — **U1** | 2 |
...
| `vision.ocr` | `FoundationModels.visionOcr(...)`; native tool `{"native": "ocr"}` | `visionOcr(params:)` — **U3** | 3 |
| `vision.barcode` | `FoundationModels.visionBarcode(...)`; native tool `{"native": "barcode"}` | `visionBarcode(params:)` — **U3** | 3 |
| `feedback.logAttachment` | `FoundationModels.logFeedbackAttachment(...)` | `logFeedbackAttachment(params:)` — **U4** | 3 |
